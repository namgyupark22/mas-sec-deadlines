...
## 업데이트 방법
새 CFP가 뜨면 `index.html`의 `CONFS` 배열에서 해당 학회의 `deadlines`(및 `dates`, `place`, `url` 등)를 수정하면 됩니다. 새 학회를 추가할 때는 `category`를 `security` / `ai3` / `ai4` / `plse` 중 하나로 지정하세요. `plse`는 "전체" 필터(`inFilter` 함수, index.html)에서 자동으로 제외됩니다.
